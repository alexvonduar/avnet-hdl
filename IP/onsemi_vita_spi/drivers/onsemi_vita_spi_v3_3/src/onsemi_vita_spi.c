...
int onsemi_vita_sensor_initialize(onsemi_vita_t *pContext, int initID, int bVerbose)
{
    ...
    if ((initID == SENSOR_INIT_SEQ00) || (initID == SENSOR_INIT_ENABLE))
    {
        ...
        if (bVerbose)
            xil_printf("VITA REMAPPER - Configuring for image lines in normal mode\n\r");
        ...
        if (bVerbose)
            xil_printf("VITA REMAPPER - Control = 0x%08X\n\r", uControl);

        if (bVerbose)
            xil_printf("VITA SPI Sequence 0 - Assert RESET_N pin\n\r");
        onsemi_vita_reset(pContext, ONSEMI_VITA_CAM_VITA_RESET_BIT);

        usleep(10); // 10 usec

        if (bVerbose)
            xil_printf("VITA SPI Sequence 0 - Releasing RESET_N pin\n\r");
        ...
        if (bVerbose)
        {
            ...
            case 0x56FA:
                ...
                break;
            case 0x50D0:
                if ((uData2 & 0x0300) == 0x0000)
                    xil_printf("\tPYTHON-1300 Sensor detected\n\r");
                if ((uData2 & 0x0300) == 0x0100)
                    xil_printf("\tPYTHON-500 Sensor detected\n\r");
                if ((uData2 & 0x0300) == 0x0200)
                    xil_printf("\tPYTHON-300 Sensor detected\n\r");
                break;
            default:
        ...
        {
            if (bVerbose)
                xil_printf("\tERROR: Absent or unsupported PYTHON sensor !!!\n\r");
            return 0;
        }
    }
    ...
    if ((initID == SENSOR_INIT_SEQ02) || (initID == SENSOR_INIT_ENABLE))
    {
        ...
        if (bVerbose)
            xil_printf("VITA SPI Sequence 2 - Verify PLL Lock Indicator\n\r");
        uAddr = 24;
        ...
        {
            if (bVerbose)
                xil_printf("\tERROR: Timed Out while waiting for PLL lock to assert !!!\n\r");
            //         return 0;
        }
    }
    ...
    if ((initID == SENSOR_INIT_SEQ06) || (initID == SENSOR_INIT_ENABLE))
    {
        ...
        if (bVerbose)
            xil_printf("VITA ISERDES - Status = 0x%08X\n\r", uStatus);
        ...
        if (bVerbose)
            xil_printf("VITA ISERDES - Waiting for CLK_RDY to assert\n\r");
        uStatus = onsemi_vita_cam_reg_read(pContext, ONSEMI_VITA_CAM_ISERDES_STATUS_REG);
        if (bVerbose)
            xil_printf("VITA ISERDES - Status = 0x%08X\n\r", uStatus);
        timeout = 9;
        ...
        if (bVerbose)
            xil_printf("VITA ISERDES - Waiting for ALIGN_BUSY to de-assert\n\r");
        uStatus = onsemi_vita_cam_reg_read(pContext, ONSEMI_VITA_CAM_ISERDES_STATUS_REG);
        if (bVerbose)
            xil_printf("VITA ISERDES - Status = 0x%08X\n\r", uStatus);
        timeout = 9;
        ...
        if (bVerbose)
            xil_printf("VITA DECODER - Control = 0x%08X\n\r", uControl);
...
int onsemi_vita_sensor_1080P60(onsemi_vita_t *pContext, int bVerbose)
{
    ...
    if (bVerbose)
        xil_printf("VITA 1080P60 - Disable Sequencer\n\r");
    ...
    if (bVerbose)
        xil_printf("VITA 1080P60 - Adjust line spacing in VITA\n\r");
    ...
    if (bVerbose)
        xil_printf("VITA 1080P60 - Tolerate 6 lines of jitter (required for programmable exposure)\n\r");
    onsemi_vita_cam_reg_write(pContext, 0x5C, 0x00000CE4);
    if (bVerbose)
        xil_printf("\tVITA_REG[0x%04X] <= 0x%08X\n\r", 0x5C, 0x00000CE4);
    ...
    if (bVerbose)
        xil_printf("VITA 1080P60 - Adjust line spacing in sync generator\n\r");
    ...
    if (bVerbose)
        xil_printf("VITA 1080P60 - Adjust frame spacing in VITA\n\r");
    ...
    if (bVerbose)
        xil_printf("VITA 1080P60 - Adjust frame spacing in sync generator\n\r");
    ...
    if (bVerbose)
        xil_printf("VITA 1080P60 - Crop ROI0 from 1920x1200 to 1920x1080\n\r");
    ...
    if (bVerbose)
        xil_printf("VITA 1080P60 - Disable auto-exposure\n\r");
    ...
    //trig internal 60 10
    if (bVerbose)
        xil_printf("VITA 1080P60 - Enable trig generator\n\r");
    {
...
//vspi rmw 192 0x0011 0x0011
   ...
   usleep(100); // 100 usec
#else
    if (bVerbose)
        xil_printf("VITA 1080P60 - Exposure related settings\n\r");
    ...
    if (bVerbose)
        xil_printf("VITA 1080P60 - Enable sequencer\n\r");
    onsemi_vita_spi_read(pContext, 192, &vspi_data);
    if (bVerbose)
        xil_printf("\tVITA_SPI[0x%04X] => 0x%04X\n\r", 192, vspi_data);
    usleep(100); // 100 usec
...
int onsemi_vita_set_exposure_time(onsemi_vita_t *pContext, Xuint32 exposureTime, int bVerbose)
{
    ...
    vitaTrigGenDefaultFreq = (((1920 + 88 + 44 + 148) * (1080 + 4 + 5 + 36)) >> 2) - 2;
    if (bVerbose)
        xil_printf("\tTrigger = internal (NA fps, duty cycle = %d \%, period = %d cycles)...\r\n", trigDutyCycle, vitaTrigGenDefaultFreq + 2);
...
int onsemi_vita_sensor_test_image(onsemi_vita_t *pContext, int initID, int bVerbose)
{
    ...
    {
        if (bVerbose)
            onsemi_vita_spi_display_sequence(pContext, vita_image_on_seq, VITA_IMAGE_ON_QTY);
    ...
    {
        if (bVerbose)
            onsemi_vita_spi_display_sequence(pContext, vita_image_off_seq, VITA_IMAGE_ON_QTY);
    ...
    {
        if (bVerbose)
            onsemi_vita_spi_display_sequence(pContext, vita_grayimage_on1_seq, VITA_GRAYIMAGE_ON_QTY);
    ...
    {
        if (bVerbose)
            onsemi_vita_spi_display_sequence(pContext, vita_grayimage_off_seq, VITA_GRAYIMAGE_OFF_QTY);
